    ...
    def __init__(self, settings):
        ...
        class Feed(Relation):
            ...
            sql = """
                ...
                FROM feeds f, _feeds_entries_join f_e, entries e,
                     entry_revisions er
        ...
        class Entry(Relation):
            ...
            def for_feed(cls, feedname):
                sql = """SELECT ep.slug, ep.published, ep.name, ep.uri,
                                ep.email, ep.term, ep.label, ep.rights,
                                ep.updated, ep.title, ep.content, ep.summary,
                                ep.tz
                         FROM entries_published ep, feeds f,
                              _feeds_entries_join fej
                         WHERE f.slug = fej.feeds_slug AND
                               fej.entries_slug = ep.slug AND
                               fej.entries_published_date = ep.published_date
                               AND f.slug = %(slug)s;"""
                cur = adb.readonly_cursor()
            ...
            def single(cls, slug, date):
                ...
                cur = adb.readonly_cursor()
                sql = """SELECT slug, published, name, uri, email, term, label,
                                rights, updated, title, content, summary, tz
